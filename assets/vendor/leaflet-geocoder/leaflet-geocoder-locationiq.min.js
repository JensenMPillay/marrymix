      ...
      function (_dereq_, module, exports) {
        (function (global) {
          (function () {
            ...
            var FULL_WIDTH_MARGIN = 20;
            var FULL_WIDTH_TOUCH_ADJUSTED_MARGIN = 4;
            var RESULTS_HEIGHT_MARGIN = 20;
            var API_RATE_LIMIT = 300;
            var TEXT_STRINGS = {
              ...
              ERROR_500:
                "The search service is not working right now. Please try again in a second.",
            ...
            var Geocoder = L.Control.extend({
              ...
              options: {
                position: "topleft",
                attribution:
                  'Search by <a href="https://locationiq.com/?ref=leaflet-geocoder">LocationIQ</a>',
              ...
              initialize: function (apiKey, options) {
                ...
                if (options && typeof options.latlng !== "undefined") {
                  ...
                  console.warn(
                    "[leaflet-geocoder-locationiq] DEPRECATION WARNING:",
                ...
                if (options && typeof options.placeholder !== "undefined") {
                  if (
                    !(
                      options.textStrings &&
                      typeof options.textStrings.INPUT_PLACEHOLDER !==
                        "undefined"
                    )
                  ) {
                    options.textStrings = options.textStrings || {};
                ...
                if (options && typeof options.textStrings === "object") {
                  for (var prop in this.options.textStrings) {
                    if (typeof options.textStrings[prop] === "undefined") {
                      options.textStrings[prop] =
                        this.options.textStrings[prop];
                    }
                  }
                }
                ...
                for (var key in params) {
                  if (params.hasOwnProperty(key)) {
                    ...
                    switch (type) {
                      case "[object Array]":
                        value =
                          param[0].toString() === "[object Object]"
              ...
              autocomplete: throttle(function (input) {
                if (!input) return;
                var url = this.options.url + "/autocomplete";
                var params = { q: input, autocomplete: 1 };
              ...
              callLocationIQ: function (endpoint, params, type) {
                ...
                function handleResponse(err, response) {
                  L.DomUtil.removeClass(
                    self._search,
                    "leaflet-locationiq-loading"
                  );
                  var results;
                  try {
                    results = JSON.parse(response.responseText);
                  } catch (e) {
                    err = { code: 404, message: "Parse Error" };
                  }
                  if (err) {
                    var errorMessage;
                    switch (err.code) {
                      ...
                      default:
                        errorMessage =
                          self.options.textStrings["ERROR_DEFAULT"];
                        break;
                    }
              ...
              getIconType: function (layer) {
                ...
                if (layer.match("venue") || layer.match("address")) {
                  if (pointIcon === true) {
                    return { type: "class", value: classPrefix + "point" };
                  } else if (pointIcon === false) {
                    return false;
                  } else {
                    return { type: "image", value: pointIcon };
                  }
                } else {
                  if (polygonIcon === true) {
                    return { type: "class", value: classPrefix + "polygon" };
                  } else if (polygonIcon === false) {
                    return false;
                  } else {
                    return { type: "image", value: polygonIcon };
                  }
                }
              },
              showResults: function (features, input) {
                ...
                resultsContainer.style.maxHeight =
                  this._map.getSize().y -
                  resultsContainer.offsetTop -
                  this._container.offsetTop -
                  RESULTS_HEIGHT_MARGIN +
                  "px";
                var list = L.DomUtil.create(
                  "ul",
                ...
                for (var i = 0, j = features.length; i < j; i++) {
                  ...
                  var resultItem = L.DomUtil.create(
                    "li",
                    "leaflet-locationiq-result",
                    list
                  );
                  ...
                  resultItem.innerHTML +=
                    "<div class='name'>" +
                    this.highlight(resultItem.name, input) +
                    "</div>" +
                    "<div class='address'>" +
                    this.highlight(resultItem.address, input) +
                    "</div>";
                }
                var branding = L.DomUtil.create(
                  "div",
                  "leaflet-locationiq-branding",
                  resultsContainer
                );
                branding.style.color = "#9c9c9c";
                branding.style.fontSize = "10px";
                branding.style.lineHeight = "15px";
                branding.style.padding = "7px";
                // branding.style.cursor = "pointer";
                branding.style.backgroundColor = "#fff";
                branding.innerHTML = "Search by LocationIQ";
                // branding.onclick = function () {
                //   window.open(
                //     "https://locationiq.com/?utm_medium=geocoder-leaf",
                //     "_blank"
                //   );
                // };
                ...
                //     branding.style.color = "#000000";
                //     branding.style.textDecoration = "underline";
                //   }
                // );
                // branding.addEventListener(
                //   "mouseout",
              ...
              showMessage: function (text) {
                ...
                resultsContainer.innerHTML = "";
                resultsContainer.style.display = "block";
                var messageEl = L.DomUtil.create(
                  "div",
              ...
              showMarker: function (text, latlng) {
                ...
                var markerOptions =
                  typeof this.options.markers === "object"
                    ? this.options.markers
                    : {};
                if (this.options.markers) {
                  var marker = new L.marker(latlng, markerOptions).bindPopup(
                    text
                  );
              ...
              setFullWidth: function () {
                if (this.options.fullWidth) {
                  ...
                  var touchAdjustment = L.Browser.touch
                    ? FULL_WIDTH_TOUCH_ADJUSTED_MARGIN
                    : 0;
                  var width = mapWidth - FULL_WIDTH_MARGIN - touchAdjustment;
                  if (
                    typeof this.options.fullWidth === "number" &&
                    mapWidth >= window.parseInt(this.options.fullWidth, 10)
                  ) {
                    this.clearFullWidth();
                    return;
                  }
              ...
              onAdd: function (map) {
                ...
                if (this.options.textStrings["INPUT_TITLE_ATTRIBUTE"]) {
                  this._input.title =
                    this.options.textStrings["INPUT_TITLE_ATTRIBUTE"];
                }
                if (this.options.textStrings["INPUT_PLACEHOLDER"]) {
                  this._input.placeholder =
                    this.options.textStrings["INPUT_PLACEHOLDER"];
                }
                this._search = L.DomUtil.create(
                  "a",
                  "leaflet-locationiq-search-icon",
                  this._container
                );
                this._reset = L.DomUtil.create(
                  "div",
                  "leaflet-locationiq-close leaflet-locationiq-hidden",
                  this._container
                );
                this._reset.innerHTML = "X";
                this._reset.title =
                  this.options.textStrings["RESET_TITLE_ATTRIBUTE"];
                this._results = L.DomUtil.create(
                  "div",
                ...
                L.DomEvent.on(
                  ...
                  .on(
                    ...
                    "keydown",
                    function (e) {
                      var list = this._results.querySelectorAll(
                        ".leaflet-locationiq-result"
                      );
                      var selected = this._results.querySelectorAll(
                        ".leaflet-locationiq-selected"
                      )[0];
                      ...
                      var panToPoint = function (selected, options) {
                        if (selected && options.panToPoint) {
                          if (selected.boundingbox && !options.overrideBbox) {
                            ...
                            self.showMarker(
                              selected.innerHTML,
                              L.latLng(selected.coords.lat, selected.coords.lon)
                            );
                          }
                        }
                      };
                      var scrollSelectedResultIntoView = function (selected) {
                        ...
                        if (selectedRect.bottom > resultsRect.bottom) {
                          self._results.scrollTop =
                            selected.offsetTop +
                            selected.offsetHeight -
                            self._results.offsetHeight;
                      ...
                      switch (e.keyCode) {
                        ...
                        case 38:
                          ...
                          this._input.value =
                            highlighted.name + ", " + highlighted.address ||
                        ...
                        case 40:
                          ...
                          var highlighted =
                            selected && nextItem ? nextItem : list[0];
                          L.DomUtil.addClass(
                            highlighted,
                            "leaflet-locationiq-selected"
                          );
                  ...
                  .on(
                    ...
                    function (e) {
                      ...
                      if (text !== this._lastValue) {
                        ...
                        if (
                          text.length >=
                            MINIMUM_INPUT_LENGTH_FOR_AUTOCOMPLETE &&
                  ...
                  .on(
                    ...
                    function (e) {
                      L.DomEvent.preventDefault(e);
                      L.DomEvent.stopPropagation(e);
                      var _selected = this._results.querySelectorAll(
                        ".leaflet-locationiq-selected"
                      )[0];
                      if (_selected) {
                        L.DomUtil.removeClass(
                ...
                L.DomEvent.on(
                  this._results,
                  "mouseover",
                  this._disableMapScrollWheelZoom,
                  this
                );
                L.DomEvent.on(
                  this._results,
